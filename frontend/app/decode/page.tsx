...
export default function DecodePage() {
  ...
  const handleJobComplete = () => {
    if (currentJob) {
      setNotification({
        ...
        message: `${currentJob.fileName || 'File'} has been restored successfully!`,
      });
    }
    setCurrentJob(null);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-secondary/30">
      {/* Background Effects */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-secondary/5 blur-[120px] rounded-full animate-pulse-slow"></div>
        <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-primary/5 blur-[120px] rounded-full animate-pulse-slow animation-delay-2000"></div>
      </div>
      ...
      <header className="border-b border-white/5 bg-slate-950/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-secondary to-primary flex items-center justify-center text-white font-bold shadow-lg shadow-secondary/20 group-hover:scale-110 transition-transform">
              f
            </div>
          ...
          <nav className="hidden md:flex items-center gap-1 bg-white/5 p-1 rounded-full border border-white/5">
            ...
            <Link href="/decode" className="px-5 py-1.5 rounded-full text-sm font-semibold transition bg-white/10 text-white shadow-sm ring-1 ring-white/10">🎥 Decode</Link>
          ...
          <div className="flex items-center gap-3">
            {activeJobs.filter(j => j.status === 'running').length > 0 && (
              <div className="hidden lg:flex items-center gap-2 px-3 py-1 bg-secondary/10 border border-secondary/20 rounded-full text-xs font-bold text-secondary animate-pulse">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 relative z-10">
          ...
          <div className="mb-8 bg-red-500/10 border border-red-500/20 rounded-2xl p-4 flex justify-between items-center animate-in fade-in slide-in-from-top-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-red-400 text-sm font-medium">
                <strong>Handled Exception:</strong> {error}
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
            ...
            <div className="space-y-12">
              <section>
                <h1 className="text-4xl lg:text-5xl font-extrabold tracking-tight text-white mb-4">
                  Reverse the <span className="text-secondary">Spectrum</span>
                </h1>
                <p className="text-slate-400 max-w-2xl text-lg leading-relaxed">
                  Extract original data from visual patterns. Our decoder performs bit-perfect
                  reconstruction with multi-stage verification.
                </p>
              </section>
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 lg:p-10 shadow-2xl relative overflow-hidden group hover:border-white/10 transition-colors">
                <div className="absolute inset-0 bg-gradient-to-br from-secondary/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>

                {currentJob ? (
                  <div className="relative z-10 animate-in fade-in zoom-in-95 duration-500">
                    <div className="flex items-center justify-between mb-8">
                      <h2 className="text-2xl font-bold text-white">Reconstruction in Progress</h2>
                      <button
          ...
          <div className="space-y-8">
            ...
            {/* Quick Info */}
            <div className="bg-gradient-to-br from-secondary/10 to-transparent p-6 rounded-2xl border border-secondary/10">
              ...
              <ul className="space-y-3 text-xs text-slate-400">
                <li className="flex justify-between">
                  <span>Method:</span>
                  <span className="text-slate-300 font-mono">Pixel-to-Bit Mapping</span>
                </li>
                <li className="flex justify-between">
                  <span>Verification:</span>
                  <span className="text-slate-300 font-mono">SHA-256 Checksum</span>
                </li>
                <li className="flex justify-between">
                  <span>Parallelism:</span>
                  <span className="text-slate-300 font-mono">SIMD Accelerated</span>
                </li>
              </ul>
            </div>
          </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-12 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 opacity-50">
        ...
        <p className="text-sm text-slate-500 font-medium">f2v2f © 2026 • Bit-Perfect Infrastructure</p>
      </footer>
    </div>
  );
}
